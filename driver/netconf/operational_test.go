...
func TestGetOperational(t *testing.T) {
	cases := map[string]*operationalTest{
		"junos-getoperational-components-fail": {
			...
			filter: `<components xmlns="http://openconfig.net/yang/platform"><component><state><type xmlns:idx="http://openconfig.net/yang/platform-types">idx:CHASSIS</type></state></component></components>`,
			shouldFail: true,
		},
	}
